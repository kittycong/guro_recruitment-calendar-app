...
function applyNoticeTemplate(xml, notice) {
  const replacements = [
    ...
    // 접수기간 본문과 1차 서류접수 표가 같은 일정값을 사용하도록 단일 날짜 치환보다 먼저 처리한다.
    [/2026\.\s*5\.\s*19\.\(화\)\s*~\s*2026\.\s*5\.\s*27\.\(수\)/g, notice.periodLine],
    [/2026\.\s*5\.\s*19\.\s*~\s*2026\.\s*5\.\s*27\./g, notice.periodLine],
